...
import sys
sys.path.append('../')
from utils import get_centre_of_bbox,measure_dist
class PlayerTracker:
    ...
    def choose_players(self,court_keypoints,player_detections_first_frame):
        ...
        for track_id,bbox in player_detections_first_frame.items():
            ...
            for i in range(0,len(court_keypoints),2):
                ...
                distance=measure_dist(player_centre,court_keypoint)
                if distance<min_dist:
                    min_dist=distance
            dist.append((track_id,min_dist))
        #DIST IN ASCENDING
        dist.sort(key=lambda x:x[1])
        chosen_players=[dist[0][0],dist[1][0]]
    ...
    def draw_bboxes(self,video_frames,player_detections):
        ...
        for frame, player_dict in zip(video_frames,player_detections):
            ...
            for track_id,bbox in player_dict.items():
                ...
                cv2.rectangle(frame,(int(x1),int(y1)),(int(x2),int(y2)),(0,255,0),2) #x,y coord and rgb color and 2 for solid border
